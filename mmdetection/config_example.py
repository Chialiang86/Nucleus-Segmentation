model = dict(
    ...
    backbone=dict(  # The config of backbone
        type='ResNet',
      # The type of the backbone, refer to
      ...
      # backbones.
        ...
        style='pytorch',
      # The style of backbone, 'pytorch' means that stride 2 layers are in 3x3
      # conv, 'caffe' means stride 2 layers are in 1x1 convs.
    	init_cfg=dict(type='Pretrained', checkpoint='torchvision://resnet50')),  # The ImageNet pretrained backbone to be loaded
    neck=dict(
        type='FPN',
      # The neck of detector is FPN. We also support 'NASFPN', 'PAFPN', etc. Refer to https://github.com/open-mmlab/mmdetection/blob/master/mmdet/models/necks/fpn.py#L10 for more details.
        # The input channels, this is consistent with the output channels of
        # backbone
    ...
    rpn_head=dict(
        type='RPNHead',
      # The type of RPN head is 'RPNHead', we also support 'GARPNHead', etc.
      # Refer to
      ...
      # Type of loss for classification branch, we also support FocalLoss etc.
            use_sigmoid=True,
      # RPN usually perform two-class classification, so it usually uses
      # sigmoid function.
    ...
    roi_head=dict(  # RoIHead encapsulates the second stage of two-stage/cascade detectors.
      ...
      # ModulatedDeformRoIPoolingPack are also supported
        ...
        mask_head=dict(  # Mask prediction head
            type='FCNMaskHead',
      # Type of mask head, refer to
      # https://github.com/open-mmlab/mmdetection/blob/master/mmdet/models/roi_heads/mask_heads/fcn_mask_head.py#L21
      # for implementation details.
            num_convs=4,  # Number of convolutional layers in mask head.
            in_channels=256,
      # Input channels, should be consistent with the output channels of mask
      # roi extractor.
            conv_out_channels=256,
      # Output channels of the convolutional layer.
            num_classes=80,  # Number of class to be segmented.
            loss_mask=dict(  # Config of loss function for the mask branch.
                type='CrossEntropyLoss',  # Type of loss used for segmentation
                use_mask=True,
      # Whether to only train the mask in the correct class.
                loss_weight=1.0))),  # Loss weight of mask branch.
    train_cfg=dict(  # Config of training hyperparameters for rpn and rcnn
        rpn=dict(  # Training config of rpn
            assigner=dict(  # Config of assigner
                type='MaxIoUAssigner',
      # Type of assigner, MaxIoUAssigner is used for many common detectors.
      # Refer to
      ...
      # for implementation details.
                ...
                pos_fraction=0.5,
      # The ratio of positive samples in the total samples.
                neg_pos_ub=-1,
      ...
      # Whether to do NMS for boxes across levels. Only work in `GARPNHead`,
      # naive rpn does not support do nms cross levels.
            nms_pre=2000,  # The number of boxes before NMS
            nms_post=1000,
      # The number of boxes to be kept by NMS, Only work in `GARPNHead`.
...
train_pipeline=[  # Training pipeline
    ...
    dict(
        ...
        # These keys are the same of img_norm_cfg since the
        mean=[123.675, 116.28, 103.53],
        # keys of img_norm_cfg are used here as arguments
        std=[58.395, 57.12, 57.375],
        to_rgb=True),
    dict(
    ...
    dict(
      ...
      # An encapsulation that encapsulates the testing augmentations
        ...
        transforms=[
            ...
            # Thought RandomFlip is added in pipeline, it is not used because
            # flip=False
...
# load models as a pre-trained model from a given path. This will not
# resume training.
load_from=None,
...
# the workflow named 'train' is executed once. The workflow trains the
